...
04_slice_views.py
-------------------
Reproduces the three 2D slice views 3D Slicer shows by default, for one
case:
...
For each plane, this script:
  1. Extracts the matching 2D pixel slice from the main T2 MR volume.
  ...
  3. Slices each biopsy tube mesh (built with a 4-sided cross-section in
     02_build_biopsy_tubes.py) with that same plane — a real plane/mesh
     intersection, same principle Slicer uses internally, so a
     diamond-like outline appears where the plane crosses the tube
     near-perpendicular, and an elongated outline where the plane runs
     close to parallel with the tube's length.

build_slice_views() takes the paths for a single case and that case's
biopsy rows so run_pipeline.py can call it once per case. Run this file
directly to build every case in one pass:
    python 04_slice_views.py

Produces, per case:
    <output_dir>/axial_view.png
    <output_dir>/sagittal_view.png
    <output_dir>/coronal_view.png
"""
...
# planes, including the coronal/sagittal plane_origin — this used to be
# missed and silently misaligned those two views), so the overlay
# always lines up correctly with the background even if the absolute
# left/right or anterior/posterior labeling doesn't exactly match a
# clinical viewer.
# ----------------------------------------------------------------------
...
def render_one_slice(image, array, meshes, tubes, plane, out_path, slice_indices):
    ...
    slice_indices: dict with keys "axial", "coronal", "sagittal" giving
        ...
        compute_centroid_slice_indices(), which centers these on the
        prostate mesh rather than the whole volume's midpoint.
    """
...
def compute_centroid_slice_indices(meshes, image):
    ...
    own centroid, rather than the whole scan volume's midpoint — the
    prostate is rarely centered in a pelvic MRI's full field of view,
    so a blind "array.shape // 2" slice can miss the gland (and every
    biopsy tube) entirely along whichever axis it's off-center on. If
    there's no prostate mesh for some reason, falls back to the volume
...
def compute_best_slice_indices(meshes, tubes, image):
    """
    Like compute_centroid_slice_indices(), but instead of fixing each
    plane at the prostate's centroid and hoping tubes happen to cross
    it, searches every candidate position within (and slightly beyond)
    the prostate's bounding box on each axis, and picks whichever
    position actually intersects the most biopsy tubes. This is what
    lets a single 2D slice show as many needle bars as a flat plane can
    possibly capture, rather than whichever tubes happen to cross one
    fixed anatomical point.

    Falls back to compute_centroid_slice_indices() if there are no
    tubes to search against (nothing to optimize for) or no prostate
    mesh at all.
    """
    if not tubes:
...
def build_slice_views(stl_dir, dicom_case_root, biopsy_rows, output_dir):
    """
    Build and save the axial/sagittal/coronal 2D slice views for one case.
    ...
    """
    main_series_folder = dicom_utils.find_main_t2_series(dicom_case_root)
    ...
    render_one_slice(image, array, meshes, tubes, "axial",    output_dir / "axial_view.png", slice_indices)
    render_one_slice(image, array, meshes, tubes, "sagittal", output_dir / "sagittal_view.png", slice_indices)
    render_one_slice(image, array, meshes, tubes, "coronal",  output_dir / "coronal_view.png", slice_indices)
