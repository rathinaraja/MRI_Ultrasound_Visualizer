...
-------------------------
Builds one pyvista "tube" mesh per biopsy core from a dataframe of
tip/base MRI coordinates + Gleason score, colored according to the
task's rule:
...
The biopsy data now lives in one shared spreadsheet covering every
case, so this module no longer reads a CSV itself — build_tubes()
takes a dataframe already filtered down to a single case's rows by
case_utils.filter_case_rows(). Run this file directly to sanity-check
tube building against the full spreadsheet's first case:
...
# Column names as given in the task. If your spreadsheet uses slightly
...
def build_tubes(df, tube_radius=1.0):
    """
    Build one pyvista tube mesh per row (biopsy core) in df.

    Parameters
    ----------
    df : pandas.DataFrame
        Already filtered to a single case's rows (see
        case_utils.filter_case_rows).
    ...
    if missing:
        raise ValueError(
            f"Could not find spreadsheet columns for: {missing}. "
    ...
    # only recorded for ~70% of cores, and missing values are filled
    # with a placeholder rather than left blank. In this spreadsheet
    # that placeholder is -1000 for the X/Y columns but +1000 for the
    # Z columns — so check the magnitude, not a fixed sign. Building a
    # tube from these draws a needle track 1000+ mm away in empty
    # space — nowhere near the prostate mesh — so these rows must be
    # dropped before building tubes, not passed through as if they
    # were real coordinates.
...
if __name__ == "__main__":
    ...
    print(f"Sanity-checking tube building against case: {first_case}")

    case_rows = case_utils.filter_case_rows(biopsy_df, first_case)
    tubes = build_tubes(case_rows)
