...
step in order, so you don't have to run six scripts by hand:
    ...
    1. visualize_scene.py  -> biopsy_visualization.png (+html)
                                 biopsy_visualization_mesh_only.png (+html)
    2. slice_views.py      -> axial_view.png, sagittal_view.png, coronal_view.png
    3. extract_highest_bvalue_nifti.py -> highest_bvalue_series.nii.gz
    4. combine_views.py    -> combined_summary.png
...
build_scene = import_module("visualize_scene").build_scene
build_mesh_only_scene = import_module("visualize_scene").build_mesh_only_scene
slice_views_main = import_module("slice_views").main
extract_nifti = import_module("extract_highest_bvalue_nifti")
build_combined_summary = import_module("combine_views").build_combined_summary
