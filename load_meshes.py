...
This case's STL folder ships TWO segmentations of the same structure —
one derived from the MR series, one from the US series (filenames like
"...ProstateSurface-seriesUID-<MR uid>.STL" AND
"...ProstateSurface-seriesUID-<US uid>.STL"), plus a third, plain-named
duplicate ("Prostate.STL") on top. Since every other part of this
pipeline (tubes, MR slice planes) works in the main MR series'
coordinate space, loading the US-derived copy too draws a second,
wrongly-positioned capsule that the tubes never actually pass through.
load_meshes() takes an optional main_series_uid and, when given, keeps
only the file(s) tagged with that series — dropping the other series'
copy and any untagged duplicate once a real match is found.

Run this file directly to sanity-check your STL files load correctly:
    python load_meshes.py
"""
...
def _select_matching_series(stl_files, main_series_uid):
    ...
    ("...-seriesUID-<uid>.STL"). Falls back to keeping everything if
    none match — safe even for a folder that doesn't use this dual-
    series naming convention at all.
    """
...
def load_meshes(stl_dir, main_series_uid=None):
    ...
    main_series_uid : str or None
        The case's main T2 MR series' SeriesInstanceUID (e.g.
        config.MAIN_MRI_SERIES_UID). When given, and the STL folder
        contains multiple series-tagged copies of the same structure,
        only the one matching this UID is kept.
...
if __name__ == "__main__":
    ...
    print(f"\nTotal prostate meshes: {len(meshes['prostate'])}")
